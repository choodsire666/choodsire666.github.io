...
```
其中 **XChecksum** 是一个四字节的整数，是对后续所有日志计算的校验和。**Log1 ~ LogN** 是常规的日志数据，**BadTail** 是在数据库崩溃时，没有来得及写完的日志数据，这个 **BadTail** 不一定存在。<br />每条日志的格式如下：
```
[Size][Checksum][Data]
