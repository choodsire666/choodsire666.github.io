...
> springMVC默认可以识别的日期字符串格式为： YYYY/MM/dd HH:mm:ss<br />通过@DateTimeFormat可以修改默认日志格式
...
> - 将/html/** 中 /**匹配到的内容，拼接到 /hhh/后<br />http://..../html/a.html  访问 /hhh/a.html
...
注意 ：1、 采用@RequestBody 首先得设置 请求头content-type:application/json<br />2、你的参数必须是json格式
...
prams: url?username=admin&pwd=123456<br />from-data: username=admin,pwd=123456
...
>  
> @JsonInclude(JsonInclude.Include.NON_NULL) //null值 属性不输出<br />@JsonInclude(value= JsonInclude.Include.NON_EMPTY) // empty属性不输出( 空串，长度为0的集合，null值)
