...
参考文章：[https://www.cnblogs.com/linianhui/p/security-https-workflow.html](https://www.cnblogs.com/linianhui/p/security-https-workflow.html)
...
启动两个浏览器，输入不同的参数([http://localhost:8080/ServletDemo/username=aaa](http://localhost:8080/ServletDemo/username=aaa) 或者bbb)，访问之后发现输出的结果都是一样，所以出现线程安全问题。
...
访问URL：[http://localhost:8080/servlet/](http://localhost:8080/servlet/) + 任何字符 
```xml
<servlet>
    <servlet-name>servletDemo</servlet-name>
    <servlet-class>com.itheima.servlet.ServletDemo</servlet-class>
...
访问URL：http://localhost:8080/任何字符任何目录 + .do ([http://localhost:8080/seazean/i.do](http://localhost:8080/seazean/i.do)) 
```xml
<servlet>
...
-  WebServlet注解（[@since ](/since ) Servlet 3.0 (Section 8.1.1)）  
```java
...
参考文章：[https://blog.csdn.net/weixin_43625577/article/details/92393581](https://blog.csdn.net/weixin_43625577/article/details/92393581)
...
-  访问URL：[http://localhost:8080/filter/servletDemo03](http://localhost:8080/filter/servletDemo03) 
-  控制台输出（注意输出顺序）： 
```
servletDemo03执行了...
filterDemo05执行了...
...
作用：增强用户和 HTML 页面的交互过程，让页面产生	动态效果，增强用户的体验。

组成部分：ECMAScript、Web APIs 
Web APIs: 
DOM操作文档，比如对页面元素的移动，大小，添加，删除等操作。
...
将<script>放在HTML文件底部附近的原因是浏览器会按照代码在文件中顺序加载HTML。
如果先加载javascript期望修改其下方HTML，那么它可能由于HTML尚未加载而失效。
JavaScript代码放在HTML页面的底部附近通常是最好的策略。
...
- 页面内容输出：document.write(“显示内容”);注：document.write("<br/>")换行，通常输出数据后跟br标签
...
- **三元运算符**
   ...
   - 格式说明：如果比较表达式为true，则取表达式1如果比较表达式为false，则取表达式2
...
| toLocaleString() | 返回本地日期格式的字符串2021/2/3下午8:20:20 |
...
- jQuery 官网：[https://www.jquery.com](https://www.jquery.com)
...
### 条件渲染
v-if：条件性的渲染某元素，判定为真时渲染，否则不渲染v-else：条件性的渲染v-else-if：条件性的渲染
...
- Element官网：[https://element.eleme.cn/#/zh-CN](https://element.eleme.cn/#/zh-CN)
- 开发步骤：
   1. 下载 Element 核心库
   2. 引入 Element 样式文件
   3. 引入 Vue 核心 js 文件
...
- 使用步骤：1.引入axios核心js文件2.调用axios对象的方法来发起异步请求3.调用axios对象的方法来处理响应的数据
- axios常用方法： 
| **方法** | **作用** |
| --- | --- |
...
# Nginx
## 安装软件
Nginx 是一个高性能的 HTTP 和[反向代理](https://baike.baidu.com/item/%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86/7793488)Web 服务器，同时也提供了 IMAP/POP3/SMTP 服务
...
6. 浏览器打开网址：[http://127.0.0.1:80](http://127.0.0.1:80)
