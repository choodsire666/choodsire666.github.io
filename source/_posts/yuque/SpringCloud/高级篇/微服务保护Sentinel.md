...
![](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/2dc45ec7a08ed5b1f86d1527ddc03725.jpeg)
## 1.2 造成雪崩问题的原因
...
![超时处理.gif](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/f648bea36aa3268a7f690d6ac57979d0.gif)
### 2 舱壁模式
限定每个业务能使用的线程数，避免耗尽整个tomcat的资源，因此也叫线程隔离。它可以避免因部分服务不可用导致整个服务不可用的问题，但是也会存在线程资源浪费的问题了。![image-20210715173215243.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/4cb6cf478afde32186a82a770cefa43c.png)
### 3 熔断降级
由断路器统计业务执行的异常比例，如果超出阈值则会熔断该业务，拦截访问该业务的一切请求。断路器会统计指定服务的请求数异常比例、异常数：![image-20210715173327075.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/838ae9527f1ba674bde237331d158ed9.png)
当发现异常比例、异常数超过配置的阈值时，断路器开始生效，拦截访问下游服务D的一切请求，形成熔断。![image-20210715173428073.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/1615989aeac7df65efda2cf595c8198e.png)
### 4 流量控制
相较于上述的针对已发生情况的自下而上的处理，实际更推荐自上而下的处理方案，这种方案将借助于Sentinel的流控功能去处理，拦截所有的请求，只释放服务能处理的粒度，从而保证服务的稳定性。
![image-20210715173555158.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/a062f17fc443b7f07607ed2f6eeeeb92.png)
## 1.4 总结
截止此我们学到了两个重要的知识点：
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/5ec6690972dffa13181593c995b54b57.png)
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/41cbefae34b2172682d404cfbc8bc4cc.png)
# 3.流量控制
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/b3c337edf4405772eb0de333da7c997e.png)
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/9f6d75ec37840b2f44ac6a9bba671682.png)
弹出表单如下，我们填写单机阈值：1，其含义是：限制资源/order/{orderId} 的QPS为1，即每秒只允许一次请求，超过的请求会被拦截并报错。
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/ccd7c2428aad724f179da3850af20a63.png)
当我们快速刷新浏览器时，会发现出现下述错误信息
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/e34db659b9b60dc86ace0b6b5c129bba.png)
但一般来说生产的流量要更多，此处我们将借助于一个压测工具：Jmeter做更为全面的压力测试。
### 1 新增限流规则
新增一个流控规则，其QPS为5：![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/40ea5e085f99bbd12997a6148ee90d86.png)
### 2 利用jmeter测试
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/466a97a9dece5dd4be130b62234f7a47.png)
确认一下：“流控入门，QPS<5”的请求路径、端口是否与自己本地一致
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/c2fd0307ddc4db2606d15f9fcc7dd914.png)
此规则如下：20个请求在2s内执行完成，此时生效的限流规则上面我们配置的为5（每秒最多5个请求通过）
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/5e9b2e6fe23f4cd7d4fc4189b6173d06.png)
右键启动运行
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/baf6ca1c1e14ed1e9a47758348a1b60c.png)
运行启动后会发现每秒能通过的请求最多5个（如下展示可能会乱序，以请求时间为准）：
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/8596f8bd38ec7401a25904ee98d46572.png)
...
![ezgif.com-gif-maker (1).gif](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/fc0d5855903d0d692ee25a238443023a.gif)
#### 关联模式如何实现
针对上述场景，我们可以建立新的关联规则如下：
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/84e34c7667d2303d96a06b3da9686cc2.png)
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/1e54f0b24c57d8585f446fca3baea331.png)
配置流控规则如下：
> 需注意：我们是针对谁做限流，就点击对应资源的流控按钮，此处我们针对：/order/query

![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/78cbf937aec86b070bcf1346c1b96214.png)
**步骤四**：
Jmeter压测验证，持续时长100s，qps=10，目的是持续造成超过配置阈值的窗口期，给用户访问验证的时间
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/ae4f29ee0d01502c1d35451ffdfe18a8.png)
运行后我们浏览器访问：[http://localhost:8088/order/query](http://localhost:8088/order/query)，发现限流成功：
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/e559c9a8e5a0dcec7a55a51fe63784d1.png)
### 3 链路模式
#### 链路模式是什么
...
![Sentinel限流-链路模式.gif](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/75acd9f283f51371fbfc5918a6702065.gif)
#### 链路模式如何实现
...
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/f8df5c08246cfdc2e9a33a56b3968d19.png)
步骤五：启动Jmeter压测，预期/oder/save不受影响，而/order/query最多允许放行流量=2
![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/6ea4ef6b9fa1b835bcaa2131935b1628.png)
## 3.3 流控效果
### 1 快速失败
#### 快速失败是什么
...
请求阈值初始值是 threshold / coldFactor，持续指定时长后，逐渐提高到threshold值。而coldFactor的默认值是3.例如，设置QPS的threshold为10，预热时间为5秒，那么初始阈值就是 10 / 3 ，也就是3，然后在5秒后逐渐增长到10。![image.png](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/0de7a9148232cb20f7d7c98a9dcdab41.png)
#### 预热模式适用于什么场景
对于大型分布式系统而言，应用启动之初需要连接多个中间件信息，此时如果大量的请求涌入，可能会让应用直接崩溃，从而无法保障系统的稳定性。对于这种模式下我们往往会采用预热模式，给系统一个缓冲时间。
![限流效果-预热模式适用场景.gif](https://raw.githubusercontent.com/choodsire666/blog-img/main/微服务保护Sentinel/8836f8dbc3ee25149e9eac05b7fc29c1.gif)
